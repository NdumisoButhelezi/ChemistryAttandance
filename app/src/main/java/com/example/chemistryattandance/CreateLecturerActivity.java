...
public class CreateLecturerActivity extends AppCompatActivity {
                            ...
                            user.updateProfile(profileUpdates)
                                    .addOnCompleteListener(profileUpdateTask -> {
                                        if (profileUpdateTask.isSuccessful()) {
                                            saveUserRole(user.getUid(), "lecturer");
                                            Toast.makeText(CreateLecturerActivity.this, "Lecturer account created successfully.", Toast.LENGTH_SHORT).show();
                                        }
                                    });
                        }
                    } else {
                        Toast.makeText(CreateLecturerActivity.this, "User creation failed.", Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    private void sendEmailIntent(String email, String password, String name) {
        ...
        emailIntent.putExtra(Intent.EXTRA_SUBJECT, "Your Lecturer Account Details");
        emailIntent.putExtra(Intent.EXTRA_TEXT, "Hello " + name + ",\n\nYour lecturer account has been created. Here are your login details:\n\nEmail: " + email + "\nPassword: " + password + "\n\nBest regards,\nAdmin");

        try {
            startActivity(Intent.createChooser(emailIntent, "Send email..."));
        } catch (android.content.ActivityNotFoundException ex) {
            Toast.makeText(CreateLecturerActivity.this, "There are no email clients installed.", Toast.LENGTH_SHORT).show();
        }
    }
}
